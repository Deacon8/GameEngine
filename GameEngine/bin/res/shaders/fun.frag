...
#define TAU             (2.0*PI)
#define RESOLUTION      vec2(500, 500);
#define TIME            time
...
vec4 plane(vec3 ro, vec3 rd, vec3 pp, vec3 off, float aa, float n) {
  ...
  float temp = mix(5.0, 30.0, h2);
  float rep = 2.0*round(temp);
...
//  float rdd = 2.0;
  ...
  const float furthest = 6.;
  const float fadeFrom = max(furthest-5., 0.);
